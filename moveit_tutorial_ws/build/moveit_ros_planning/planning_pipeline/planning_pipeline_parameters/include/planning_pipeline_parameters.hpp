...
[[nodiscard]] auto to_parameter_value(rsl::StaticVector<T, capacity> const& value) {
    ...
    struct Params {
        std::string planning_plugin = "UNKNOWN";
  ...
  public:
    ...
    rcl_interfaces::msg::SetParametersResult update(const std::vector<rclcpp::Parameter> &parameters) {
      ...
      for (const auto &param: parameters) {
        if (param.get_name() == (prefix_ + "planning_plugin")) {
            updated_params.planning_plugin = param.as_string();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
        if (param.get_name() == (prefix_ + "request_adapters")) {
            updated_params.request_adapters = param.as_string();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
      }
    ...
    void declare_params(){
      ...
      if (!parameters_interface_->has_parameter(prefix_ + "planning_plugin")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "Name of the planner plugin used by the pipeline";
          descriptor.read_only = false;
          auto parameter = to_parameter_value(updated_params.planning_plugin);
          parameters_interface_->declare_parameter(prefix_ + "planning_plugin", parameter, descriptor);
      }
      if (!parameters_interface_->has_parameter(prefix_ + "request_adapters")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "Names of the planning request adapter plugins (plugin names separated by space).";
          descriptor.read_only = false;
      ...
      rclcpp::Parameter param;
      param = parameters_interface_->get_parameter(prefix_ + "planning_plugin");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      updated_params.planning_plugin = param.as_string();
